 ...
 *
 * Adapted from the work of St. John Johnson and Jeremiah Wuenschel
 " Added capability to update ST sensor data from the bridge
 */
...
metadata {
    definition (name: "DH.MQTT Bridge", namespace: "juancal", author: "juancal") {
...
def setNetworkAddress() {
    ...
    if (device.deviceNetworkId != "$hex") {
        ...
        log.debug "MQTT bridge: Device Network Id set to ${device.deviceNetworkId}"
    }
}
...
def parse(String description) {
    ...
    log.debug "MQTT bridge:parse:Parsing '${description}'"
    def msg = parseLanMessage(description)

    log.debug "MQTT bridge:parse:creating an event to ST:"
    log.debug "MQTT bridge:parse:name: message, value: '${msg.data}' "
...
def deviceNotification(message) {
    if (device.hub == null)
    {
        log.error "MQTT bridge: Hub is null, must set the hub in the device settings so we can get local hub IP and port"
        return
    }
    
    log.debug "MQTT bridge: Sending '${message}' to device"
    setNetworkAddress()
